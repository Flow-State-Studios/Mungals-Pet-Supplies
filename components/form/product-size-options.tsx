...
const ProductSizeOptions = ({product_id, variation_size, available_sizes}: {product_id: string, variation_size: any, available_sizes: any[]}) => {
            ...
            <div className={`${styles.section_options}`}>
                {
                    available_sizes.map((size: any, idx: number) => {
                        ...
                        return  <Link href={`/shop/${product_id}?size=${size.size_in_kg}&color=${color}`} key={size.id}>
                            <label className={`radio_label`} tabIndex={0}>
                                {size.size_in_kg} kgs
                                <input 
                                    type={`radio`} name='size' required={true}
